package alternativa.gui.focus {
	...
	public class FocusManager implements IFocusManager {
		...
		public function set focus(target:DisplayObject):void {
			...
			if (target != null) {
				if (target is IFocus) {
					if (_focused != IFocus(target)) {
						if (IFocus(target).tabEnabled) {
							newFocus = IFocus(target);
						} else {
		...
		private function onFocusOut(e:FocusEvent):void {
			...
			if (mouseFocusChanged || keyFocusChanged) {
				// Менеджеры клавиатуры и мыши сами установят новый фокус (set focus) и фокусное дерево перестроится
			} else {
				// Вся флэшка потеряла фокус
				clearTree();
			}
		}
